...
vec3f pathtrace_ray(Scene* scene, ray3f ray, Rng* rng, int depth) {
    //float threshold = .5;
    //Russian Roulette
    //if(rng->next_float() < threshold) return zero3f;
   ...
   // if((not (intersection.hit))|| (depth==scene->path_max_depth && depth >0)) {
    ...
    //bool dof = false;
    //if(dof){
     //   auto aperatureSize = .5;
      //  auto pixSize = .5;
       // auto dofRay = ray3f(intersection.pos, ray.d); //WHAT TO DO HERE?
        //auto q = ray.d;

       // if(dofRay.e.x + aperatureSize * (.5 - rng->next_float()) > 0 && dofRay.e.x + aperatureSize * (.5 - rng->next_float()) < scene->camera->width){
        //    dofRay.e.x += aperatureSize * (.5 - rng->next_float());
       // }
       // if(dofRay.e.y + aperatureSize * (.5 - rng->next_float()) > 0 && dofRay.e.y + aperatureSize * (.5 - rng->next_float()) < scene->camera->height){
        //    dofRay.e.y += aperatureSize * (.5 - rng->next_float());
       // }

        //////dofRay.d.x += pixSize * (.5 - rng->next_float());
       //// //dofRay.d.y += pixSize * (.5 - rng->next_float());

       // int N = 3; //samples

        //auto dofSum = vec3f();
        //for(int i=0; i<N; i++){
        //    dofSum += pathtrace_ray(scene,dofRay, rng, depth+1);
       // }
        ...
        for(auto i = 0; i < scene->image_width; i ++) {
            ...
            for(auto jj : range(scene->image_samples)) {
                for(auto ii : range(scene->image_samples)) {
                    ...
                    //Depth of Field
                    auto dof = false; //Depth of Field
                    if(dof){
                        auto aperatureSize = .5;
                        auto pixSize = .5;
                        auto dofRay = ray; //WHAT TO DO HERE?
                        //auto q = ray.d;
                        ...
                        auto dofSum = vec3f();
                        for(int i=0; i<N; i++){
                            dofSum += pathtrace_ray(scene,dofRay, rng, 0);
                        }

                        image->at(i,j) += dofSum/N;
                    }
                }
            }
