...
vec3f pathtrace_ray(Scene* scene, ray3f ray, Rng* rng, int depth) {
    ...
    for(Surface * s: scene->surfaces){
        ...
        vec3f kes = lookup_scaled_texture(intersection.mat->ke, intersection.mat->ke_txt, intersection.texcoord, true);
        ...
        // compute the material response (brdf*cos)
        vec3f material_repsonse = max(dot(light_norm,ldir),0.0f)*eval_brdf(kd, ks, n, v, ldir, norm, mf);
